...
function AdminWhatsapp() {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase.from("site_settings").select("*").in("key", ["whatsapp_group", "contact"]);
      (data ?? []).forEach((r) => {
  ...
  async function save() {
    setSaving(true);
    setMsg(null);
    const contactRow = await supabase.from("site_settings").select("value").eq("key", "contact").maybeSingle();
  ...
  if (loading) return <div className="grid min-h-[40vh] place-items-center"><Loader2 className="h-6 w-6 animate-spin text-sage-deep" /></div>;
    ...
    <div className="p-6 md:p-10">
      ...
      <section className="rounded-2xl border border-border bg-card p-6 shadow-soft">
        ...
        <p className="mb-4 text-sm text-muted-foreground">
          Se o link do grupo estiver vazio ou a seção estiver desativada, o botão não aparece no site.
        </p>
        <label className="mb-4 inline-flex items-center gap-2 text-sm">
          <input type="checkbox" checked={group.active} onChange={(e) => setGroup({ ...group, active: e.target.checked })} />
          Ativar seção
        </label>
        <div className="grid gap-4">
          <Field label="Título" value={group.title} onChange={(v) => setGroup({ ...group, title: v })} />
          <Field label="Descrição" value={group.description} onChange={(v) => setGroup({ ...group, description: v })} textarea />
          <Field label="Texto do botão" value={group.cta} onChange={(v) => setGroup({ ...group, cta: v })} />
          <Field label="Link de convite do grupo (chat.whatsapp.com/...)" value={group.link} onChange={(v) => setGroup({ ...group, link: v })} />
          <div className="flex flex-wrap gap-4 text-sm">
            <label className="inline-flex items-center gap-2">
              <input type="checkbox" checked={group.show_on_home} onChange={(e) => setGroup({ ...group, show_on_home: e.target.checked })} />
              Mostrar na Home
            </label>
            <label className="inline-flex items-center gap-2">
              <input type="checkbox" checked={group.show_on_contact} onChange={(e) => setGroup({ ...group, show_on_contact: e.target.checked })} />
...
function Field({
  ...
  return (
    <label className="block">
      ...
      {textarea ? (
        <textarea value={value} onChange={(e) => onChange(e.target.value)} rows={3} className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:border-sage" />
      ) : (
