...
const SECTIONS: { key: string; title: string; fields: { key: string; label: string; type?: "text" | "textarea" }[] }[] = [
  {
    key: "contact",
    title: "Contato",
    fields: [
...
export default function AdminSettings() {
  ...
  async function load() {
    ...
    (data ?? []).forEach((r) => { map[r.key] = { key: r.key, value: (r.value ?? {}) as Record<string, unknown> }; });
    SECTIONS.forEach((s) => { if (!map[s.key]) map[s.key] = { key: s.key, value: {} }; });
    setRows(map);
    setLoading(false);
  }
  useEffect(() => { load(); }, []);
