...
function AdminBlog() {
  ...
  async function load() {
    ...
    if (error) {
      setMsg("Não foi possível carregar os posts. Tente novamente.");
    } else {
  ...
  async function save(p: Post) {
    ...
    if (p.id) {
      const { error } = await supabase
        ...
        .single();
      if (error) {
        setMsg("Não foi possível salvar o post. Verifique os dados e tente novamente.");
        setSaving(false);
        return;
      }
    } else {
      ...
      if (error) {
        setMsg("Não foi possível criar o post. Verifique os dados e tente novamente.");
        setSaving(false);
        return;
      }
  ...
  async function togglePublish(p: Post) {
    ...
    const publishedAt =
      status === "published" ? (p.published_at ?? new Date().toISOString()) : p.published_at;
    ...
    const { error } = await supabase
      ...
      .single();
    if (error) {
      setMsg("Não foi possível alterar a publicação do post. Tente novamente.");
    } else {
      setPosts((prev) =>
        prev.map((post) =>
          post.id === p.id ? { ...post, status, published_at: publishedAt } : post,
        ),
      );
      setMsg(
        status === "published" ? "Post publicado com sucesso." : "Post despublicado com sucesso.",
      );
    }
  ...
  async function remove(p: Post) {
    ...
    const { error } = await supabase
      ...
      .single();
    if (error) {
      setMsg("Não foi possível excluir o post. Tente novamente.");
    } else {
      setPosts((prev) => prev.filter((post) => post.id !== p.id));
      if (editing?.id === p.id) setEditing(null);
      setMsg("Post excluído com sucesso.");
    }
    setProcessingId(null);
  }
    ...
    return (
      <div className="p-6 md:p-10">
        <button
          ...
          className="mb-4 inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-sage-deep"
        >
          <ArrowLeft className="h-4 w-4" /> Voltar para lista
        </button>
        <div className="mb-6 flex items-center justify-between">
          <h1 className="font-serif text-3xl text-sage-deep">
            {editing.id ? "Editar post" : "Novo post"}
          </h1>
          <button onClick={() => save(editing)} disabled={saving} className="btn-serena">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="space-y-4 lg:col-span-2">
            ...
            />
            <Field
              label="Conteúdo (aceita Markdown)"
          ...
          <div className="space-y-4">
            ...
            <Field
              ...
              onChange={(v) => setEditing({ ...editing, cover_image_url: v })}
            />
  ...
  return (
    <div className="p-6 md:p-10">
      <div className="mb-6 flex items-center justify-between">
        ...
        <button onClick={newPost} className="btn-serena">
                ...
                <tr key={p.id} className="border-t border-border">
                  <td className="px-4 py-3">
                    <button
                      ...
                      className="text-left font-medium text-sage-deep hover:underline"
                    >
                      {p.title}
                    </button>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-3">
                      <button
                        ...
                        className="text-xs text-sage-deep hover:opacity-70"
                      >
                        {processingId === p.id ? (
