...
interface SerenaChatProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function SerenaChat({ open, onOpenChange }: SerenaChatProps) {
      ...
      <div className="flex items-center justify-between border-b border-border/60 bg-cream/80 px-5 py-4">
        <div className="flex items-center gap-3">
          <div className="relative inline-flex h-10 w-10 items-center justify-center rounded-full bg-sage-deep text-gold">
            ...
            <span
              className="absolute inset-0 rounded-full ring-1 ring-gold/30 animate-breathe"
              aria-hidden
            />
          </div>
          <div>
        ...
        <button
          ...
          className="inline-flex h-9 w-9 items-center justify-center rounded-full text-foreground/60 hover:bg-blush transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sage focus-visible:ring-offset-2"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      <div className="border-t border-border/60 bg-cream/60 px-3 py-3">
        <div className="flex items-center gap-2">
          <input
            ...
            onKeyDown={(e) =>
              e.key === "Enter" && !e.shiftKey && (e.preventDefault(), sendText(input))
            }
